<script lang="ts">
  ...
  // Add Developer menu item only for the developer email
  let developerNavItems = $derived([
    ...navItems,
    ...(authStore.user?.email === DEVELOPER_EMAIL ? [{ name: 'Developer', path: '/developer', icon: 'Terminal' }] : [])
  ]);
    ...
    >
      <div class="h-full overflow-y-auto flex flex-col">
        <nav class="p-4 space-y-2 flex-1">
          {#each developerNavItems as item}
            <button
